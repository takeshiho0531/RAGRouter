...
def predict_top5(model, tokenizer, device, target_jsonl_data, query_map):
    ...
    for data in tqdm(target_jsonl_data, desc="Predicting Top-5"):
        ...
        with torch.no_grad():
            ...
            # get probabilities of label=1.0 (rag correct) through sigmoid
...
def main(args):
    ...
    print("=== Training Final Model for All-Data Prediction ===")
    full_train_ds = CEFoldDataset(rag_df, query_map, doc_map, tokenizer)
    final_model = train_one_fold(full_train_ds, device, model_name, args)

    print("=== Predicting All Data for Test-Feature Generation ===")
    all_scores = predict_top5(final_model, tokenizer, device, all_jsonl, query_map)
    all_output_path = f"data/{args.data_type}/ce_score_top5_all.csv"
    pd.DataFrame(all_scores).to_csv(all_output_path, index=False)

    print(f"All query scores saved to {all_output_path}")
